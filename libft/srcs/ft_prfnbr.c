...
/*   Updated: 2018/08/30 19:34:22 by vgladush         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	void	prfsec(char c, int *i, va_list *ar, char *ito)
{
	...
	else if (i[2] == 2 && (i[1] += write(i[19], "+", 1)))
		i[1] += write(i[19], ito, ft_strlen(ito));
	else if (i[2] == 1 && (i[1] += write(i[19], " ", 1)))
		i[1] += write(i[19], ito, ft_strlen(ito));
	else
		i[1] += write(i[19], ito, ft_strlen(ito));
	free(ito);
...
void			ft_prfnbr(char c, int *i, va_list *ar, char *ito)
{
	if ((c == 'd' || c == 'i' || c == 'D') && (i[13] = 1))
	{
		...
		if (i[10] > (int)ft_strlen(ito) || i[11] > -1)
			ito = endprf(ito, i, ft_strlen(ito), c);
		else if (i[2] == 2 && *ito != '-' && (i[1] += write(i[19], "+", 1)))
			i[1] += write(i[19], ito, ft_strlen(ito));
		else if (i[2] == 1 && *ito != '-' && (i[1] += write(i[19], " ", 1)))
			i[1] += write(i[19], ito, ft_strlen(ito));
		else
			i[1] += write(i[19], ito, ft_strlen(ito));
		free(ito);
