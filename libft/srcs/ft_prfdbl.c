...
/*   Updated: 2018/08/30 19:33:29 by vgladush         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libft.h"

static int		justfree(int i, char **ito, int a, char c)
{
	if (!ito)
	{
		if (c == 'G' || c == 'E')
...
static	char	*enddbl(char *s, int *i, int r, int l)
{
	if (l != -1)
		r += 4;
	if (*s != '-' && (i[2] == 2 || (i[2] == 1 && (i[1] += write(i[19], " ", 1)))))
		i[10] -= 1;
	else if (*s == '-' && i[3] == 1 && (i[14] = 1))
		i[1] += write(i[19], "-", 1);
	if (i[3] == 2 && i[2] == 2 && *s != '-' && (i[1] += write(i[19], "+", 1)))
		i[1] += write(i[19], &s[i[14]], ft_strlen(&s[i[14]]));
	else if (i[3] == 2)
		i[1] += write(i[19], &s[i[14]], ft_strlen(&s[i[14]]));
...
void			ft_prfdbl(char c, int *i, va_list *ar)
{
	...
	if (i[11] && (i[1] += write(i[19], ".", 1)))
		i[1] += write(i[19], &ito[1][1], ft_strlen(&ito[1][1]));
	if (l != -1 && (i[1] += 4))
		justfree(i[19], 0, l, c);
	justfree(i[19], ito, 2, 0);
}
